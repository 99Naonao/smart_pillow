...
const resolveAesLib = (aesjs) => {
  const queue = [aesjs]
  if (typeof globalThis !== 'undefined') queue.push(globalThis.aesjs)
  const visited = new Set()
  while (queue.length) {
    const c = queue.shift()
    const t = typeof c
    if (!c || (t !== 'object' && t !== 'function') || visited.has(c)) continue
    visited.add(c)
    if (c.ModeOfOperation && c.ModeOfOperation.ofb) return c
    if (c.default && (typeof c.default === 'object' || typeof c.default === 'function')) {
      queue.push(c.default)
    }
    if (c._aesjs && (typeof c._aesjs === 'object' || typeof c._aesjs === 'function')) {
      queue.push(c._aesjs)
    }
    ...
    for (let i = 0; i < keys.length; i++) {
      const v = c[keys[i]]
      const vt = typeof v
      if (v && (vt === 'object' || vt === 'function')) queue.push(v)
    }
  }
  return null
}
...
const blueAesEncrypt = (aesjs, mdKey, iv, bytes) => {
  const aesLib = resolveAesLib(aesjs)
  if (!aesLib) {
    throw new Error('aesjs library unavailable: ModeOfOperation.ofb missing')
  }
  var aesOfb = new aesLib.ModeOfOperation.ofb(mdKey, iv);
